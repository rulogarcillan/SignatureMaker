...
object Utils {
    ...
    fun loadAllPreferences(mContext: Context?) {
        mContext?.let {
            ...
            if (mContext.loadSharedPreference(Constants.ID_PREF_STROKE, false)) {
                minStroke = mContext.loadSharedPreference(Constants.PREF_MIN_STROKE, Constants.DEFAULT_MIN_STROKE)
                maxStroke = mContext.loadSharedPreference(Constants.PREF_MAX_STROKE, Constants.DEFAULT_MAX_STROKE)
            } else {
                defaultStroke()
            }
    ...
    fun saveAllPreferences(mContext: Context?) {
        mContext?.let {
            ...
            if (mContext.loadSharedPreference(Constants.ID_PREF_STROKE, false)) {
                mContext.saveSharedPreference(Constants.PREF_MIN_STROKE, minStroke)
                mContext.saveSharedPreference(Constants.PREF_MAX_STROKE, maxStroke)
            }
            if (mContext.loadSharedPreference(Constants.ID_PREF_WALLPAPER, false)) {
