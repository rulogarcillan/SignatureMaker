...
object Utils {
    ...
    fun loadAllPreferences(mContext: Context) {
        ...
        if (SharedPreferencesRepository.loadPreference(mContext, Constants.ID_PREF_WALLPAPER, false)) {
            wallpaper = SharedPreferencesRepository
    ...
    fun shareSign(mActivity: Activity, name: String) {
        ...
        val imageUri: Uri = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.N) {
            Uri.parse(File("$path/$name").toString())
        } else {
            Uri.fromFile(File("$path/$name"))
        }
        ...
        if (name.contains(".png") || name.contains(".PNG")) {
            ...
            val uri = FileProvider
                .getUriForFile(mActivity, BuildConfig.APPLICATION_ID, File("$path/$name"))
    ...
    fun sort(list: List<ItemFile>, type: Int) {
        sort(list) { (_, date1), (_, date2) ->
            val formatter = SimpleDateFormat("dd/MM/yyyy HH:mm:ss")
            val ddate: Date? = formatter.parse(date1)
            val ddate2: Date? = formatter.parse(date2)
            ddate?.compareTo(ddate2)!! * type
        }
    }
}